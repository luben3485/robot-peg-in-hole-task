...
def main():
    ...
    date = '2021-06-20'
    anno_data = 'xyz_track_curve_insertion_' + date + '/processed'
    im_data = 'xyz_track_curve_insertion_' + date + '/processed/images'
    ...
    for track_idx in range(iter):
        ...
        gripper_pose = rob_arm.get_object_matrix(obj_name='UR5_ikTarget')
        delta_x = random.uniform(-0.12, 0.12)
        delta_y = random.uniform(-0.12, 0.12)
        delta_z = random.uniform(0.12, 0.17)
        #delta_x, delta_y = random_gripper_xy(ran_hole_pos)

        move_vec = np.array([delta_x, delta_y, delta_z])
        start_pos = gripper_pose[:3,3]
        target_pos = start_pos + move_vec
        if move_record:
            '''
            step_size = 0.04
            ...
            horizontal_distance = np.linalg.norm((target_pos - start_pos)[:2])
            while sum_move <= distance :
                # get current position
                gripper_pose = rob_arm.get_object_matrix(obj_name='UR5_ikTarget')
                current_pos = gripper_pose[:3, 3]
                # compute move direction
                move_dir = (target_pos - current_pos) * 0.05
                if np.linalg.norm(current_pos - start_pos) < horizontal_distance * 0.2:
                    move_dir[2] += random.uniform(horizontal_distance*0, horizontal_distance*0.5)
                    print('HOYO')
                elif np.linalg.norm(current_pos - start_pos) >= horizontal_distance * 0.2:
                    pass
                # check z of current position is less than z of target position
                #if (current_pos + move_dir)[2] > target_pos[2]:
                #    print('???')
                #    move_dir = target_pos - current_pos
                ...
                j+=1
                delta_rotation_quat = np.array([1.0, 0.0, 0.0, 0.0])
                delta_translation = - move_dir * step_size * const_factor
                gripper_pose = rob_arm.get_object_matrix(obj_name='UR5_ikTarget')
                state_idx = -1
