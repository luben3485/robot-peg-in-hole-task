...
import mankey.network.resnet_nostage_xyzrot_cls as resnet_nostage
...
def construct_resnet_nostage(ckpnt_path):
    ...
    if device == 'cpu':
        state_dict = torch.load(ckpnt_path, map_location=torch.device('cpu'))
    else:
        state_dict = torch.load(ckpnt_path)
...
def inference_resnet_nostage(
    ...
    # gripper pose is not used here
    if enable_gripper_pose == True:
    ...
    # Do forward
    xy_depth_pred, delta_rot_x_pred, delta_rot_y_pred, delta_rot_z_pred, delta_xyz_pred, step_size_pred = network(stacked_rgbd, gripper_pose, device, enableKeypointPos=enableKeypointPos)
    '''
    ...
    coord_x = xy_depth_pred[:, :, 0].cpu().detach().numpy()
    coord_y = xy_depth_pred[:, :, 1].cpu().detach().numpy()
    depth_pred = xy_depth_pred[:, :, 2].cpu().detach().numpy()

    # Combine them
    num_keypoints = xy_depth_pred.shape[1]
    keypointxy_depth_pred = np.zeros((3, num_keypoints))
    keypointxy_depth_pred[0, :] = coord_x[0, :]
    keypointxy_depth_pred[1, :] = coord_y[0, :]
    keypointxy_depth_pred[2, :] = depth_pred[0, :]

    # classification
    delta_rot_x_pred = delta_rot_x_pred.argmax(dim=-1).cpu().detach().numpy()[0]
    delta_rot_y_pred = delta_rot_y_pred.argmax(dim=-1).cpu().detach().numpy()[0]
    delta_rot_z_pred = delta_rot_z_pred.argmax(dim=-1).cpu().detach().numpy()[0]
    # regression
    delta_xyz_pred = delta_xyz_pred.cpu().detach().numpy()[0]
    step_size_pred = step_size_pred.cpu().detach().numpy()[0]

    return keypointxy_depth_pred, delta_rot_x_pred, delta_rot_y_pred, delta_rot_z_pred, delta_xyz_pred, step_size_pred
