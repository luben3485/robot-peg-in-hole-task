...
def proc_input_img_internal(
    ...
    # do gray scale here
    #normalized_gray = np.dot(np.transpose(normalized_rgb, (1, 2, 0)), [0.2989, 0.5870, 0.1140])
    ...
    # Construct the tensor for grayscale
    #channels, height, width = normalized_gray.shape
    #stacked_gray = np.zeros(shape=(channels + 1, height, width), dtype=np.float32)
    #stacked_gray[0:channels, :, :] = normalized_gray
    #channel_offset = channels
    #stacked_gray[channel_offset, :, :] = normalized_depth

    # Construct the tensor for rgb
    channels, height, width = normalized_rgb.shape
    stacked_rgbd = np.zeros(shape=(channels + 1, height, width), dtype=np.float32)
    stacked_rgbd[0:channels, :, :] = normalized_rgb
    channel_offset = channels
    stacked_rgbd[channel_offset, :, :] = normalized_depth
