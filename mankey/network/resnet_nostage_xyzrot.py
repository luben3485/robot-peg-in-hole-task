...
class ResnetNoStage(nn.Module):
    ...
    def forward(self, x, gripper_pose, device, enableGripperPose=True):
        x_feature = self.backbone_net(x)
        print(x_feature.shape)
        x_heatmap = self.head_net(x_feature)
        x_heatmap_flatten = torch.flatten(x_heatmap, start_dim=1)
        if enableGripperPose == True:
            _out = self.mlp_2(x_heatmap_flatten)
            gripper_pose_flatten = torch.flatten(gripper_pose[:,:3,:], start_dim=1)
            _out = torch.cat((_out, gripper_pose_flatten), dim=1)
            out = self.mlp_3(_out)
        else:
            out = self.mlp_1(x_heatmap_flatten)
        out_r_6d = out[:, 0:6]
        ...
        return x_heatmap, out_r, out_t, out_step_size
