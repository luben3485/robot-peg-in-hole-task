...
def construct_dataset(is_train: bool) -> (torch.utils.data.Dataset, SupervisedKeypointDatasetConfig):
    ...
    if is_train:
        db_config.config_file_path = '/home/luben/robotic-arm-task-oriented-manipulation/mankey/config/insertion_20210126.txt'
    else:
        db_config.config_file_path = '/home/luben/robotic-arm-task-oriented-manipulation/mankey/config/insertion_20210126.txt'
...
def train(checkpoint_dir: str, start_from_ckpnt: str = '', save_epoch_offset: int = 0):
    ...
    # And the dataloader
    loader_train = DataLoader(dataset=dataset_train, batch_size=64, shuffle=True, num_workers=4)
    ...
    for epoch in range(n_epoch):
        ...
        for idx, data in enumerate(loader_train):
            ...
            # Compute loss
            depth_loss = weighted_l1_loss(depth_pred, keypoint_xy_depth[:, :, 2], keypoint_weight[:, :, 2])
            heatmap_loss = heatmap_criterion(heatmap, target_heatmap)
            np.save('pred_heatmap.npy', heatmap.cpu().detach().numpy())
            np.save('target_heatmap.npy', target_heatmap.cpu().detach().numpy())
            if idx % 100 == 0:
            ...
            depth_error = float(weighted_l1_loss(depth_pred, keypoint_xy_depth[:, :, 2], keypoint_weight[:, :, 2]).item())
            if idx % 100 == 0:
                ...
                print('The averaged depth error is (mm): ', train_config.depth_image_scale * depth_error / len(depth_pred))
...
if __name__ == '__main__':
    ...
    net_path = 'ckpnt1/checkpoint-100.pth'
