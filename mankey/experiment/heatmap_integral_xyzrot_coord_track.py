...
from mankey.dataproc.xyzrot_supervised_db_track import SpartanSupvervisedKeypointDBConfig, SpartanSupervisedKeypointDatabase
from mankey.dataproc.supervised_keypoint_loader_track import SupervisedKeypointDatasetConfig, SupervisedKeypointDataset
...
def construct_dataset(is_train: bool) -> (torch.utils.data.Dataset, SupervisedKeypointDatasetConfig):
    ...
    if is_train:
        db_config.config_file_path = '/tmp2/r09944001/robot-peg-in-hole-task/mankey/config/insertion_20210822.txt'
    else:
        db_config.config_file_path = '/tmp2/r90944001/robot-peg-in-hole-task/mankey/config/insertion_20210822.txt'
...
def train(checkpoint_dir: str, start_from_ckpnt: str = '', save_epoch_offset: int = 0):
    ...
    # And the dataloader
    loader_train = DataLoader(dataset=dataset_train, batch_size=1, shuffle=True, num_workers=1)
...
if __name__ == '__main__':
    ...
    checkpoint_dir = os.path.join(os.path.dirname(__file__), 'ckpnt_xyzrot_coord_small_range_0822_no_kpt')
