...
def construct_dataset(is_train: bool) -> (torch.utils.data.Dataset, SupervisedKeypointDatasetConfig):
    ...
    if is_train:
        db_config.config_file_path = '/tmp2/r09944001/robot-peg-in-hole-task/mankey/config/insertion_20210719.txt'
    else:
        db_config.config_file_path = '/tmp2/r90944001/robot-peg-in-hole-task/mankey/config/insertion_20210719.txt'
...
def train(checkpoint_dir: str, start_from_ckpnt: str = '', save_epoch_offset: int = 0):
    ...
    for epoch in range(n_epoch):
        ...
        for idx, data in enumerate(loader_train):
            ...
            raw_pred, delta_rot_pred, delta_xyz_pred, step_size_pred = network(image, device)
...
if __name__ == '__main__':
    ...
    checkpoint_dir = os.path.join(os.path.dirname(__file__), 'ckpnt_xyzrot_0719')
