...
def main():
    ...
    for key, value in data.items():
        ...
        if key==0:
            ...
            tmp_pcd.points = o3d.utility.Vector3dVector(np.array([(hole_keypoint_top_pos_in_world+hole_keypoint_top_pose_in_world[:3,1]*0.065)*1000, hole_keypoint_bottom_pos_in_world*1000]).reshape(-1,3))
            o3d.io.write_point_cloud(os.path.join(data_root, 'tmp_' + str(key) + '.ply'), tmp_pcd)

            x_normal_vector = hole_keypoint_top_pose_in_world[:3, 0]
            x_1 = np.dot(x_normal_vector, hole_keypoint_top_pos_in_world + [0., 0., 0.002])
            ...
            # and y >= y_value[0] and y <= y_value[1] and z >= z_value[0] and z <= z_value[1]
