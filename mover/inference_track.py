...
import torch
import numpy as np
from .models.model_track import Model
...
class MoveInference():
    ...
    def inference_single_frame(self, rgb=None, depth_mm=None , rgb_path='', depth_mm_path=''):
        ...
        self.model.eval()
        with torch.no_grad():
            #out_r, out_t, out_step, out_next_t = self.model(rgbd.to(device), torch.randn(1,3,1).float().to(device))
            out_r, out_t, out_step = self.model(rgbd.to(device), torch.randn(1, 3, 1).float().to(device))
        out_r = torch.squeeze(out_r).detach().cpu().numpy()  # (3,3)
        out_t = torch.squeeze(out_t).detach().cpu().numpy()  # (3, )
        out_step = torch.squeeze(out_step).detach().cpu().numpy()  # (3, )
        #out_t[0] /= 66 # scale x
        #out_t[1] /= 40  # scale y
        #out_t[2] /= 33  # scale z
