...
from models.model_track import Model, init_from_modelzoo
...
checkpoint_dir = os.path.join(os.path.dirname(__file__), 'ckpnt_xyz_track_curve_0620')
    ...
    for epoch in range(1, n_epoch+1):
        ...
        for idx, (rgbd_track, gt_r_track, gt_t_track, gt_step_track, gt_pos_track, gt_next_t_track) in enumerate(progress):
            ...
            for episode in range(num_episode):
                ...
                #print('gt_t',gt_t)
                out_r, out_t, step = model(rgbd.to(device),gt_pos.to(device))
                ...
                loss_t = criterion_rmse_t(out_t, gt_t.to(device))+(1-criterion_cos(out_t, gt_t.to(device))).mean() 
                #loss_t = loss_t[:,0] * weight_x +  loss_t[:,1] * weight_y +  loss_t[:,2] * weight_z 
                #loss_t = loss_t.mean()
                loss = loss_t * w_t + loss_step * w_s
