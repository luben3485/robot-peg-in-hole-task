...
netpath = '/Users/cmlab/robot-peg-in-hole-task/mankey/experiment/ckpnt_box_fix_3_grayscale/checkpoint-100.pth'
...
def main():
    ...
    depth = rob_arm.get_depth(cam_name=cam_name, near_plane=0.01, far_plane=far_plane)
    ...
    while err_size_rolling > err_tolerance:
        ...
        rgb = rob_arm.get_rgb(cam_name=cam_name)
        depth = rob_arm.get_depth(cam_name=cam_name, near_plane=near_plane, far_plane=far_plane)
        # rotate 180
        (h, w) = rgb.shape[:2]
        center = (w / 2, h / 2)
        M = cv2.getRotationMatrix2D(center, 180, 1.0)
