...
def generate_one_im_anno(cnt, cam_name, peg_top, peg_bottom, hole_top, hole_bottom, rob_arm, im_data_path):
    ...
    rot_matrix = robot_pose[:3, :3]
    pos = robot_pose[:3, 3]
    quat = mat2quat(rot_matrix)
    rot_vec, rot_theta = quat2axangle(quat)
    ...
    for i in range(iter):
        ...
        if pick_record:
            ...
            for j in range(int(1 / step_size)):
                ...
                #print('pick step:' + str(j))
                print('iter: '+str(i)+' cnt: '+str(cnt)+' pick: '+str(j))
                info = generate_one_im_anno(cnt, cam_name, peg_top, peg_bottom, hole_top, hole_bottom, rob_arm, im_data_path)
                info_dic[cnt] = info
                cnt += 1
        else:
        ...
        if tilt_record:
            ...
            for j in range(int(1 / step_size)):
                ...
                gripper_pose[:3, :3] = rot_matrix
                rob_arm.movement(gripper_pose)
                print('iter: ' + str(i) + ' cnt: ' + str(cnt) + ' rotate: ' + str(j))
                info = generate_one_im_anno(cnt, cam_name, peg_top, peg_bottom, hole_top, hole_bottom, rob_arm,im_data_path)
                info_dic[cnt] = info
                cnt += 1
        ...
        if move_record:
            ...
            step_size = 0.1
            print('iter: ' + str(i) + ' cnt: ' + str(cnt) + ' move: ' + str(0))
            info = generate_one_im_anno(cnt, cam_name, peg_top, peg_bottom, hole_top, hole_bottom, rob_arm, im_data_path)
            info_dic[cnt] = info
            cnt += 1
            for k in range(int(1/step_size)):
                if k==0:
                    for l in range(10):
                        ...
                        rob_arm.movement(gripper_pose)
                        print('iter: ' + str(i) + ' cnt: ' + str(cnt) + ' move: ' + str(l+1))
                        info = generate_one_im_anno(cnt, cam_name, peg_top, peg_bottom, hole_top, hole_bottom, rob_arm,im_data_path)
                        info_dic[cnt] = info
                        cnt += 1
                        if l == 9:
                    ...
                    rob_arm.movement(gripper_pose)
                    print('iter: '+str(i)+' cnt: ' + str(cnt) + ' move: ' + str(k+10))
                    info = generate_one_im_anno(cnt, cam_name, peg_top, peg_bottom, hole_top, hole_bottom, rob_arm, im_data_path)
                    info_dic[cnt] = info
                    cnt += 1
        else:
