...
def scale_point_cloud(data, hole_top_pose, seg_label, min_scale=0.8, max_scale=1.2):
    ...
    """
    scales =  np.random.uniform(min_scale, max_scale, (2,))
    ...
    for idx, xyz in enumerate(data):
        if seg_label[idx] == 1 and np.linalg.norm(hole_top_pose[:3, 3]*1000 - xyz) > 30:
...
def parse_args():
    ...
    parser.add_argument('--aug_data', action='store_true', default=False, help='data augmentation')
...
def main(args):
    ...
    for key, value in tqdm.tqdm(list(data.items())):
        ...
        if args.crop_pcd == True:
            ...
            seg_label = seg_label[index_list]
            seg_color = seg_color[index_list]
