...
from train_dataset_bykovis import DataServoStereo
import train_model_bykovis as model
...
cvt = model.ConverterServo(arg.num_keypoint * 1 * 3, arg.growth_rate[2], arg.blk_cfg_cvt, [sum(arg.motion_vec), 1]).cuda()
# original input shape is arg.num_keypoint * 2 * 3, but we lony use one image here.
...
def train(ep, loader_train):
    for i, (inL0, outDL,outSL, vecM, intV) in enumerate(loader_train):
        # data
        inL0 = inL0.cuda()
        outDL = outDL.cuda()
        outSL = outSL.cuda()
        ...
        lossD = F.smooth_l1_loss(depthL, outDL, reduction='none').view(arg.batch_size, -1).topk(boot_recon, sorted=False)[0].mean() 
        
        lossS = F.cross_entropy(segL, outSL, reduction='none').view(arg.batch_size, -1).topk(boot_recon, sorted=False)[0].mean()

        # motion
        #vec, speed = cvt(torch.cat((keypL0[0], keypR0[0]), dim=1))
        vec, speed = cvt(keypL0[0])
        if vecM.shape[1] <= 3:
            lossM = F.cosine_similarity(vec, vecM).mul(-1).add(1).mul(intV).\
                topk(boot_batch, sorted=False)[0].mean()
        else:
            lossM_t = F.cosine_similarity(vec[:, :3], vecM[:, :3]).mul(-1).add(1).mul(intV).\
                topk(boot_batch, sorted=False)[0].mean()
            lossM_r = F.mse_loss(vec[:, 3:], vecM[:, 3:]).mul(intV).\
                topk(boot_batch, sorted=False)[0].mean()
            lossM = lossM_t + lossM_r
...
def save_checkpoint(base_dir, ep):
    state = {'enc_state_dict': enc.state_dict(),
             'dec_state_dict': dec.state_dict(),
             'cvt_state_dict': cvt.state_dict()}
    torch.save(state, os.path.join(base_dir, 'ckpt_' + str(ep) +'.pth'))
    #print('checkpoint saved.')


def load_checkpoint(base_dir):
    cp_net = torch.load(os.path.join(base_dir, 'ckpt_23.pth'))
...
def test(loader_test):
    ...
    sims, speeds = [], []
    for i, (inL, _, _, vecM, intV) in enumerate(loader_test):
        inL = inL.cuda()
        vecM = vecM.cuda()
        intV = intV.cuda()
...
def main():
    if arg.task in ['full']:
        ...
        for ep in trange(arg.ep_train):
            ...
            # save
            save_checkpoint(arg.dir_base, ep)
