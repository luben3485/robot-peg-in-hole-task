...
class DataServoStereo(_data.Dataset):
    def __init__(self, arg, train=True):
        ...
        self.ims = sorted(listdir(path.join(self.data_path, 'front/color')))
    ...
    def data_transform(self, index):
        ...
        # label
        '''
        idx = int(self.ims[index].split('.')[0])
        gt = self.gts[idx]
        ...
        intV = 1 - np.exp(gt[-1] * self.a_dist)
        
        return inL0, outDL, outSL, vecM, intV
...
def img_proc_servo(data_path, im, im_size, obj_class,
    ...
    # output depth
    outD = Image.open(path.join(data_path, 'depth_uint8', im))
