...
class DataServoStereo(_data.Dataset):
    def __init__(self, arg, train=True):
        ...
        self.ims = sorted(listdir(path.join(self.data_path, 'front/color'))) # image file name in left and right folder is the same.
    ...
    def data_transform(self, index):
        # images
        inL0, outDL, outSL = img_proc_servo(path.join(self.data_path, 'front'), self.ims[index],
...
def img_proc_servo(data_path, im, im_size, obj_class,
    ...
    outD = (outD - min_value) / (max_value - min_value)
    outD = torch.tensor(outD, dtype=torch.float32).unsqueeze(0)
...
def img_proc(data_path, idx_bg, im_size,
    ...
    inC.sub_(mean[:, None, None]).div_(std[:, None, None])
