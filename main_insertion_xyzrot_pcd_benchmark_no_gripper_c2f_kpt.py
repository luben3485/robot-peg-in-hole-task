...
from inference_pointnet2_kpts import Mover
...
def predict_kpts_no_oft_from_multiple_camera(cam_name_list, gripper_pose, mover, rob_arm):
    ...
    mean_kpt_pred, mean_kpt_x_pred, mean_kpt_y_pred, rot_mat_pred, confidence = mover.test_network(points)
    real_kpt_pred = (mean_kpt_pred * pcd_mean) + pcd_centroid # unit:mm
...
def main():
    ...
    benchmark_folder = 'pcd_benchmark/kpts_no_oft'
    if not os.path.exists(benchmark_folder):
        os.makedirs(benchmark_folder)
    #f = open(os.path.join(benchmark_folder, "benchmark_0103_0220_reg_vision_eye_concat_c2f_kpt.txt"), "w")
    ...
    #coarse_mover = Mover(model_path='kpts/2022-03-15_18-30', model_name='pointnet2_kpts', checkpoint_name='best_model_e_28.pth', use_cpu=False, out_channel=9)
    fine_mover = FineMover(model_path='kpt/2022-03-04_18-23', model_name='pointnet2_kpt', checkpoint_name='best_model_e_45.pth', use_cpu=False, out_channel=9)
    #fine_mover = Mover(model_path='kpts/2022-03-15_18-52', model_name='pointnet2_kpts', checkpoint_name='best_model_e_42.pth', use_cpu=False, out_channel=9)
    iter_num = 500
    ...
    for selected_hole in selected_hole_list:
        ...
        for iter in range(iter_num):
            ...
            if abs(r_euler[0]) < 90 and abs(r_euler[1]) < 90 and abs(r_euler[2]) < 90:
                ...
                #gripper_pose[:3, 3] += np.array(rot_matrix[:3, 0]*0.04) # tmp for kpts_no_oft
            ...
            while True:
                ...
                gripper_pose = rob_arm.get_object_matrix('UR5_ikTip')
                delta_xyz_pred = predict_kpt_xyz_from_multiple_camera(cam_name_list, gripper_pose, fine_mover, rob_arm)
                #delta_xyz_pred, _ = predict_kpts_no_oft_from_multiple_camera(cam_name_list, gripper_pose, fine_mover, rob_arm)
