...
from inference_pointnet2_reg_seg_heatmap import CoarseMover, FineMover
...
def predict_xyzrot_from_multiple_camera(cam_name_list, mover, rob_arm):
    ...
    delta_rot_pred, delta_xyz_pred, step_size_pred = mover.inference_multiple_camera(depth_mm_list, camera2world_list)
...
def main():
    ...
    #benchmark_folder = 'pcd_benchmark/reg'
    if not os.path.exists(benchmark_folder):
        os.makedirs(benchmark_folder)
    f = open(os.path.join(benchmark_folder, "benchmark_0112_0112_reg_seg_heatmap_v3_vision_eye_concat_c2f_closeloop.txt"), "w")
    #f = open(os.path.join(benchmark_folder, "benchmark_0101_1230_reg_vision_eye_concat_c2f.txt"), "w")

    rob_arm = SingleRoboticArm()
    init_pose = rob_arm.get_object_matrix(obj_name='UR5_ikTarget')

    model_path_coarse = 'reg_seg_heatmap_v2/2022-01-18_03-48'
    model_path_fine = 'reg_seg_heatmap_v3/2022-01-20_02-18'
    coarse_mover = CoarseMover(model_path=model_path_coarse, model_name='pointnet2_reg_seg_heatmap_msg_v2', checkpoint_name='best_model_e_83.pth', use_cpu=False, out_channel=9)
    fine_mover = FineMover(model_path=model_path_fine, model_name='pointnet2_reg_seg_heatmap_msg_v3', checkpoint_name='best_model_e_200.pth', use_cpu=False, out_channel=9)

    #coarse_mover = PointnetMover(date='2022-01-03_14-47', model_name='pointnet2_reg_msg', checkpoint_name='best_model_e_115.pth',use_cpu=False, out_channel=9)
    #fine_mover = PointnetMover(date='2022-01-03_14-44', model_name='pointnet2_reg_msg', checkpoint_name='best_model_e_115.pth',use_cpu=False, out_channel=9)
    ...
    for iter in range(iter_num):
        ...
        # coarse approach
        delta_rot_pred, delta_xyz_pred, _ = predict_xyzrot_from_multiple_camera(cam_name_list, coarse_mover, rob_arm)
        ...
        while True:
            ### start
            delta_rot_pred, delta_xyz_pred, step_size_pred = predict_xyzrot_from_multiple_camera(cam_name_list, fine_mover, rob_arm)
            print(step_size_pred)
            gripper_pose = rob_arm.get_object_matrix('UR5_ikTip')
            rot_matrix = np.dot(gripper_pose[:3, :3], delta_rot_pred[:3, :3])
            gripper_pose[:3, :3] = rot_matrix
            gripper_pose[:3, 3] += delta_xyz_pred
            rob_arm.movement(gripper_pose)
            if step_size_pred < 0.004 or cnt >= 5:
                break
            cnt = cnt+1
